...
STRUCTURES = ("spike", "stonewall", "woodwall", "turret")
SOLID_OBJECTS = ("spike", "stonewall", "woodwall", "turret", "stone", "tree", "bush")
...
-----------------------------------------------------------------------------------------------------------------------------
STRATEGY:
    - For each agent, figure out its priorities and goals
    - Script will figure out what each agent should do based on what's happening on the map and what each agent wants
    - Agent will recieve instructions from script and attempt to carry them out
...
class StrongAgent(BaseAgent):
    ...
    def handleAgentPriority(self, id_):
        '''
        MOVE:
            individual agent does not prioritize this
        FOLLOW: 
            follow other agent, too vulnerable to be by itself
        FIND_RESOURCE: 
            explore to find resource, no higher priorities on the map
        FIND_ENEMY: 
            early in the game and looking for quick kills, late in the game and hunting stragglers
        GATHER_RESOURCE: 
            gather resources nearby, no higher priorities on the map
        BUILD_DEFENCE: 
            surplus of resources, build up base
        EXPLORE_BASE: 
            nothing else to do, just wander base
        CLEAR_ENEMY_DEFENCE: 
            no immediate threat, clear out enemy defences like spikes and turrets
        DEFEND:
            prevent enemy from approaching
        ATTACK: 
            approach enemy
        '''
        pass
    ...
    def handleTeamObservation(self, team_observation):
        self.team_observation = team_observation
    ...
    def handleTeamObservationsRaider(self, team_observations):
        '''
        For each raider, figure out its goals and priorities
